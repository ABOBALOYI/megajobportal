...
if(isset($_POST['login'])){
    $_SESSION['user_email_id'] = $_POST['email_address'];
    ...
    if(!empty($_POST['email_address']) && !empty($_POST['password'])){
       ...
       echo $sql_query = "SELECT * FROM employer_registration WHERE emplr_email_address = '".$_SESSION['user_email_id']."' AND emplr_password = '".$_SESSION['password']."'";
        $result = mysqli_query($connection,$sql_query);
       
        $row = mysqli_fetch_assoc($result);
        
            if($user_name == $row['emplr_email_address'] && $password == $row['emplr_password']) {
            
              $_SESSION['user_session_info'] = array();
              $_SESSION['user_session_info']["emplr_email_address"] = $row["emplr_email_address"];
              $_SESSION['user_session_info']["emplr_user_name"] = $row["emplr_user_name"];
              $_SESSION['user_session_info']["emplr_id"] = $row["emplr_id"]; 
            
              //print_r($_SESSION['user_session_info']); die;
                if($row['emplr_email_address'] == $row['emplr_email_address']){
                header('location: employer_dashboard/index.php');
                  }
                }else{
...
<html lang="en">
  <head>
    ...
    <title>USAEAD Employer | Login</title>
